...
outputs.dir.bs <- "../outputs/bootstrapping/"
...
resval.scaled.wc.df <- resval.scaled.wc.df %>%
  ...
  mutate(category.focal = factor(category.focal,
                                 levels = c("urban", "water", "agric", "beach", "scrub", "trees", "opend", "all_else")))
#write file to make combined plots of run 1 and run 2
write.csv(resval.scaled.wc.df, paste0(outputs.dir.su, "resistance_values_scaled_run1.csv"))
...
resval.scaled.ad.df <- resval.scaled.ad.df %>%
  ...
  mutate(category.focal = factor(category.focal,
                                 levels = c("urban", "water", "agric", "beach", "scrub", "trees", "opend", "all_else")))
#write file to make combined plots of run 1 and run 2
write.csv(resval.scaled.ad.df, paste0(outputs.dir.su, "resistance_values_scaled_AD_run1.csv"))
...
#westcoast MC----
##model averaging====
su.wc.mc <- read.csv(paste0(outputs.dir.su, "summary_westcoast_MC_run2_27122022.csv"), sep=",", header=T) %>% rename("surface"="Surface") #load summary results
bs.wc.mc <- read.csv(paste0(outputs.dir.bs, "ResultsBootstrapping_westcoast_run2_1000it_SS_MC_27122022.csv"), sep=",", header=T) #load bootstrapping results
...
resval.scaled.wc.df <- data.frame()#initialize dataframe
combs_cat <- readRDS(file= paste0(data.dir, "vector-combinations/comb_", "westcoast", "_run2.RDS")) #load possible combinations of categories
for(cat.focal in categories.v){
  print(cat.focal)
  ...
  for(cats in combs_cat){
    if(grepl(cat.focal, cats)){ #if cat.focal is present in current combination
      ...
      #create column with the categories of this surface in
      ifelse(cats == "opend.fixed.scrub.beach.agric.water.urban", 0, 1)
      if(cats == "opend.fixed.scrub.beach.agric.water.urban"){
        cats.df <- as.data.frame(str_split(cats, pattern="[.]")) %>% rename ("category" = 1) 
      } else{
        cats.df <- as.data.frame(str_split(paste0("all_else.", cats), pattern="[.]")) %>% rename ("category" = 1) #create column with the categories of this surface in
      }
      # print(cats.df)
      # print(row.focal %>% pivot_longer(cols = starts_with("Feature")) %>% drop_na)
      #get data of value and categories toegether
      data.model <- row.focal %>% pivot_longer(cols = starts_with("Feature")) %>% drop_na %>% bind_cols(cats.df) 
      #get value of focal category, and then rescale according to this value
      value.focal.cat <- as.numeric(data.model %>% dplyr::filter(category==cat.focal) %>% dplyr::select(value))  
...
resval.scaled.wc.df <- resval.scaled.wc.df %>%
  ...
  mutate(category.focal = factor(category.focal,
                                 levels = c("urban", "water", "agric", "beach", "scrub", "trees", "opend", "all_else")))
#write file to make combined plots of run 1 and run 2
write.csv(resval.scaled.wc.df, paste0(outputs.dir.su, "resistance_values_scaled_run2.csv"))
...
##model averaging====
areasdunes.v <- c("westhoek", "cabour", "doornpanne", "teryde")
su.ad.mc.all <- read.csv(paste0(outputs.dir.su, "/summary_areasdunes_MC_run2_27122022.csv"), sep=",", header=T) %>% rename("surface"="Surface") #load summary results

ad.mc.all <- data.frame() #initialize dataframe
for(areadune in areasdunes.v){
  bs.ad.mc <- read.csv(paste0(outputs.dir.bs, "/ResultsBootstrapping_", areadune, "_run2_1000it_SS_MC_27122022.csv"), sep=",", header=T) #load bootstrapping results
  su.ad.mc <- su.ad.mc.all %>% dplyr::filter(dunearea==areadune)
...
for(areadune in areasdunes.v){
  print(areadune)
  combs_cat <- readRDS(file= paste0(data.dir, "vector-combinations/comb_", areadune, "_run2.RDS")) #load possible combinations of categories
  ...
  for(cat.focal in categories.v){
    ...
    for(cats in combs_cat){
      if(grepl(cat.focal, cats)){ #if cat.focal is present in current combination
        ...
        row.focal <- ad.mc.all %>% filter(area==areadune & surface==cats) %>% dplyr::select(surface, starts_with("Feature"))
        #create column with the categories of this surface in
        if((areadune=="westhoek" & cats == "opend.fixed.scrub.beach.urban") | (areadune=="cabour" & cats=="opend.fixed.scrub.agric.water.urban") |
           ((!areadune %in% c("westhoek", "cabour")) & cats=="opend.fixed.scrub.beach.agric.water.urban")){
          cats.df <- as.data.frame(str_split(cats, pattern="[.]")) %>% rename("category" = 1)
        } else{
          cats.df <- as.data.frame(str_split(paste0("all_else.", cats), pattern="[.]")) %>% rename("category"=1)
        }
...
resval.scaled.ad.df <- resval.scaled.ad.df %>%
  ...
  mutate(category.focal = factor(category.focal,
                                 levels = c("urban", "water", "agric", "beach", "scrub", "trees", "opend", "all_else")))
#write file to make combined plots of run 1 and run 2
write.csv(resval.scaled.ad.df, paste0(outputs.dir.su, "resistance_values_scaled_AD_run2.csv"))
