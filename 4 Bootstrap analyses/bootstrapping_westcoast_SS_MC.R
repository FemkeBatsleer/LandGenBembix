###Aim: westcoast bootstrapping post hoc from results run on hpc###
#by: Femke Batsleer
...
if(hpc.bool==TRUE){
  ...
  resolution <- 50
  data.dir <- "./data/" #hpc
  runs.SC.dir <- "./data/outputs/outputs_SS_SC_westcoast_17102022/"
  runs.MC.dir <- "./data/outputs/outputs_SS_MC_westcoast_31102022/"
  output.dir <- "./data/outputs/bootstrapping/" #hpc #
} else{
  resolution <- 50
  n.iter <- 2 #should be 1000, but runs for long!
  data.dir <- "D:/Fbatslee/LandGenData/Github-files/" #local
  runs.SC.dir <- "D:/Fbatslee/LandGenData/outputs/outputs_SS_SC_westcoast_17102022/"#define folders where outputs are situated
...
dist.v <- readRDS(file = paste0(data.dir, "genetic_distances/westcoast/westcoast_PCA16.RDS"))
#coordinates of the data
coord.samples <- readRDS(file = paste0(data.dir, "genetic_distances/westcoast/westcoast_coordsamples.RDS"))
...
for(category in categories.v){
  #print(category)
  results.dir <- paste0(runs.SC.dir, "westcoast", "/", resolution, "m/", category, "/Results/results.gdist.RDS")
...
for(category in combs_cat){
  #print(category)
  results.dir <- paste0(runs.MC.dir, "westcoast", "/", resolution, "m/", category, "/Results/results.gdist.RDS")
  if(!file.exists(results.dir)){
    print(paste0(results.dir, " does not exist"))#if category not present, go to next iteration
    next}
  #load results-rds to get cost/resistance distance matrices
...
tic(paste0("bootstrapping westcoast MC"))
...
toc()
AIC.boot
write.csv(AIC.boot, file = paste0(output.dir, "ResultsBootstrapping_westcoast_", n.iter, "it_", "SS_MC_31102022.csv"))
