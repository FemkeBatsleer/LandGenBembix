...
if(hpc.bool==TRUE){
  ...
  resolution <- 50
  data.dir <- "./data/" #hpc
  runs.dir <- "./data/outputs/outputs_SS_SC_westcoast_17102022/"
  output.dir <- "./data/outputs/bootstrapping/" #hpc #
} else{
  resolution <- 50
  n.iter <- 2
  data.dir <- "D:/Fbatslee/LandGenData/Github-files/" #local
  runs.dir <- "D:/Fbatslee/LandGenData/outputs/outputs_SS_SC_westcoast_17102022/"#define folders where outputs are situated
...
for(category in categories.v){
  cat.dir <- paste0(runs.dir, "westcoast", "/", resolution, "m/", category)
...
tic(paste0("bootstrapping westcoast SC"))
...
toc()
AIC.boot
write.csv(AIC.boot, file = paste0(output.dir, "ResultsBootstrapping_westcoast_", n.iter, "it_", "SS_SC_17102022.csv"))
